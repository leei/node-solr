...
Client.prototype.update = function(data, options, callback) {
  ...
  if (Object.keys(options).length > 0) {
    cmd += '?' + querystring.stringify(options);
  }
  this.post(cmd, data, callback);
};
...
Client.prototype.add = function(doc, options, callback) {
  ...
  this.update(data, addParams, callback);
};
  ...
  if (id) {
    if (Array.isArray(id)) {
      for (var i=0; i<id.length; i++) {
        data = data + "<id>" + escapeXml(id[i]) + "</id>";
      }
    } else {
      data = data + "<id>" + escapeXml(id) + "</id>";
    }
  }
  if (query) {
    if (Array.isArray(query)) {
      for (var i=0; i<query.length; i++) {
        data = data + "<query>" + escapeXml(query[i]) + "</query>";
      }
    } else {
      data = data + "<query>" + escapeXml(query) + "</query>";
    }
  }
  data = data + "</delete>";
...
Client.prototype.commit = function(params, callback) {
  ...
  var data = "<commit/>";
  this.update(data, params, callback);
};

Client.prototype.optimize = function(params, callback) {
if (typeof params == 'function') {
    callback = params; params = {};
  }
  var data = "<optimize/>";
